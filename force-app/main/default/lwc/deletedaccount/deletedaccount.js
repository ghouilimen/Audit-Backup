...
import BackupAcc from '@salesforce/apex/BackupHelper.BackupAcc';
import getdeletedacc from '@salesforce/apex/AuditActions.getdeletedacc';
import getupdateacc from '@salesforce/apex/AuditActions.getupdateacc';
import RollBackUpdateAcc from '@salesforce/apex/BackupHelper.RollBackUpdateAcc';
...
export default class Deletedproduct extends NavigationMixin(LightningElement)  {
    ...
    handleclicks(event){
        ...
        BackupAcc({ sObj: row }).then(result => {
          this.loading = true;
      ...
      handleclickss(event){
        const row = event.detail.row;
        RollBackUpdateAcc({ sObj: row }).then(result => {
          this.loading = true;
          this.stopLoading(800);
        }).then(result => {
          const evt = new ShowToastEvent({
    ...
    @wire(getdeletedacc, {  searchkey: '$searchkey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    ...
    @wire(getupdateacc, {  searchkey: '$searchkey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    getupdateacc({ error, data }) {
        if (data) {
            console.log(data)
