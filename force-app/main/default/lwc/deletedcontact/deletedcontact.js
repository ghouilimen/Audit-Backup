...
import BackupContact from '@salesforce/apex/BackupHelper.BackupContact';
import getdeletedcon from '@salesforce/apex/AuditActions.getdeletedcon';
import getupdatecon from '@salesforce/apex/AuditActions.getupdatecon';
import RollBackUpdateContact from '@salesforce/apex/BackupHelper.RollBackUpdateContact';
...
export default class Deletedproduct extends NavigationMixin(LightningElement)  {
    ...
    handleclicks(event){
        ...
        BackupContact({ sObj: row }).then(result => {
          this.loading = true;
      ...
      handleclickss(event){
        const row = event.detail.row;
        RollBackUpdateContact({ sObj: row }).then(result => {
          this.loading = true;
          this.stopLoading(800);
        }).then(result => {
          const evt = new ShowToastEvent({
    ...
    @wire(getdeletedcon, {  searchkey: '$searchkey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    ...
    @wire(getupdatecon, {  searchkey: '$searchkey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    getupdatecontact({ error, data }) {
        if (data) {
            console.log(data)
