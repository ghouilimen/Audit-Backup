...
import RollBackUpdatelead from '@salesforce/apex/BackupHelper.RollBackUpdatelead';
import getUpdateLead from '@salesforce/apex/AuditActions.getUpdateLead';
...
export default class Deletedproduct extends NavigationMixin(LightningElement)  {
    ...
    handleclicks(event){
      ...
      handleclickss(event){
        const row = event.detail.row;
        RollBackUpdatelead({ sObj: row }).then(result => {
          this.loading = true;
          this.stopLoading(800);
        }).then(result => {
          const evt = new ShowToastEvent({
    ...
    @wire(getUpdateLead, {  searchkey: '$searchkey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    getUpdateLead({ error, data }) {
        if (data) {
            console.log(data)
