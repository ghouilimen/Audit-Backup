...
import Backupopp from '@salesforce/apex/BackupHelper.BackupOpp';
import RollBackUpdateopp from '@salesforce/apex/BackupHelper.RollBackUpdateopp';
import getupdateopp from '@salesforce/apex/AuditActions.getupdateopp';
import getdeletedopp from '@salesforce/apex/AuditActions.getdeletedopp';
...
export default class Deletedproduct extends NavigationMixin(LightningElement)  {
    ...
    handleclicks(event){
        ...
        Backupopp({ sObj: row }).then(result => {
          this.loading = true;
      ...
      handleclickss(event){
        const row = event.detail.row;
        RollBackUpdateopp({ sObj: row }).then(result => {
          this.loading = true;
          this.stopLoading(800);
        }).then(result => {
          const evt = new ShowToastEvent({
    ...
    @wire(getdeletedopp, {  searchkey: '$searchkey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    ...
    @wire(getupdateopp, {  searchkey: '$searchkey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    getupdateopp({ error, data }) {
        if (data) {
            console.log(data)
