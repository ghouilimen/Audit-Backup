...
import BackupPro from '@salesforce/apex/BackupHelper.BackupPro';
import RollBackUpdatepro from '@salesforce/apex/BackupHelper.RollBackUpdatepro';
import getdeletedpro from '@salesforce/apex/AuditActions.getdeletedpro';
import getupdatepro from '@salesforce/apex/AuditActions.getupdatepro';
...
export default class Deletedproduct extends NavigationMixin(LightningElement)  {
    ...
    handleclicks(event){
        ...
        BackupPro({ sObj: row }).then(result => {
          this.loading = true;
      ...
      handleclickss(event){
        const row = event.detail.row;
        RollBackUpdatepro({ sObj: row }).then(result => {
          this.loading = true;
          this.stopLoading(800);
        }).then(result => {
          const evt = new ShowToastEvent({
    ...
    @wire(getdeletedpro, {  searchkey: '$searchkey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    ...
    @wire(getupdatepro, {  searchkey: '$searchkey', sortBy: '$sortedBy', sortDirection: '$sortedDirection'})
    getupdatepro({ error, data }) {
        if (data) {
            console.log(data)
